...
static t_token redirection(t_lexer *const lexer)
{
	...
	if (strchr("<>&", next_char))
	{
		...
		{
			token.value = strndup(&lexer->input[lexer->input_idx], 2);
			lexer->input_idx += 2;
			return token;
		}
	}
	token.value = strndup(&lexer->input[lexer->input_idx], 1);
	lexer->input_idx++;
	return token;
}
...
static t_token ambiguous_operator(t_lexer *const lexer)
{
	...
	if (next_char == '|')
	{
		t_token token = {
			.type = Logical,
			.value = strndup(&lexer->input[lexer->input_idx], 2)
		};
		lexer->input_idx += 2;
		return token;
	}
	...
	t_token token = {
		.type = Pipe,
		.value = strndup(&lexer->input[lexer->input_idx], 1)
	};
	lexer->input_idx++;
	return token;
}
